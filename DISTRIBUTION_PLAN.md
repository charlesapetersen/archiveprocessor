...
**Phase 3 (iPhone capture companion) — `DONE` (build + runtime-launch verified + reviewed; device signing/run = `[USER]`). REFRAMED (lower-risk):** Option A (capture companion, chosen) needs almost **no shared core** — it streams photos to the Mac's existing `CaptureServer` and holds **no API keys**. So build it as a **new, self-contained iOS project `ArchiveCaptureiOS/`** (XcodeGen), mirroring the Android companion + the segment-transfer UX — **no `ArchiveCore` package extraction and zero changes to the working Mac app** (that heavy refactor only pays off for a *standalone* iOS processor, Option B). Env: iOS SDK 26.2 + xcodegen present → can **compile-verify** (simulator SDK, generic destination, `CODE_SIGNING_ALLOWED=NO`); **cannot runtime-test** here (no booted simulator/device) — running + signing are `[USER]`.
- [x] **3a** scaffolded `ArchiveCaptureiOS/` (XcodeGen; Info.plist camera/local-network/Bonjour strings). ⚠️ Swift **5** language mode for this target (AVFoundation vs strict-6 concurrency); Mac app stays Swift 6.
- [x] **3b** net: `MacEndpoint` (QR JSON parse) + `MacClient` (ping / postPhoto raw-JPEG+X-* headers+Bearer / sessionComplete) — mirrors Android + the `CaptureServer` contract (always sends non-empty X-Group + numeric X-Seq).
- [x] **3c** `CaptureModels` + `SessionStore` (durable JSON) + `CaptureViewModel` — full segment-transfer UX ported from Android (remove-on-Mac-confirm with identity guard, transfer flash, PENDING-marker recovery, restore-purge).
- [x] **3d** UI: `CameraController` (AVFoundation photo) + `CameraPreview` + `QRScannerView` + `ConnectScreen` (QR + manual) + `CaptureScreen` (preview, Box/shutter/Folder, End segment, transfer strip/flash, tag sheet) + `ContentView` switch. ✅ **full app compiles clean** (iphonesimulator SDK).
- [x] **3e** runtime smoke test on a booted sim (iPhone 17, iOS 26.3): builds + installs + launches; **ConnectScreen renders** (QR + manual) → app launch verified. Adversarial review (5 agents: durability/protocol/camera-ui + 2 verifiers) → 2 CONFIRMED camera defects, both FIXED + rebuilt clean:
  - **HIGH** overlapping shutter taps overwrote a single shared `captureCompletion` → silently dropped a page (violated "never lose a photo") + cross-queue data race → fixed with a per-request `[uniqueID: completion]` map confined to the serial queue, plus a UI `isCapturing` in-flight guard on the capture buttons.
  - **MED** `configureIfNeeded` marked the session `configured` even when no camera input was added → `capturePhoto` would throw `NSInvalidArgumentException` (crash) → fixed: track `hasVideoInput` + guard on a live `.video` connection, converting to graceful `completion(nil)`.
**Phase 3 DONE.** (Device run + code-signing remain `[USER]` — can't sign/run on hardware here.)
**NEXT ACTION:** Phase 4 (store submission). Mostly `[USER]`: Apple Developer + Play Console accounts, signing, screenshots/GIFs, submission. Claude drafts privacy policy / Data-Safety / App-Privacy labels on request.
