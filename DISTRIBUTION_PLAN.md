...
- [ ] **3e** runtime smoke test on a booted sim (ConnectScreen renders; camera N/A in sim) + adversarial review + commit + push
Env: iOS 26.3 simulator runtime installed (iPhone 17 etc. available) → can now also runtime-smoke-test the app launch.
**NEXT ACTION:** 3e — boot a sim, launch + screenshot; adversarial review; push. Then Phase 4 (store submission = `[USER]`).
