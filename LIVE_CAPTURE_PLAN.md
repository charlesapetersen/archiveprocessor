...
- **Mac-side segment tagging during capture (uncommitted, after v3.2.1):** an **auto-advancing tag card** — as each completed document segment arrives, a sheet pops in `LiveCaptureView` (`SegmentTagCard`) showing its pages + a **Subjects** field (`TagInputField` autocomplete) + editable year/month/priority pre-filled from the phone. Save stores them (`CaptureSession.macTags` / `resolvedGroupIds`, surfaced via `pendingTagGroup`); Skip advances. Subjects ride the handoff (`…Subjects` arrays) into the run; in **automatic** tagging, Mac-tagged segments apply those subjects and **skip the LLM call**, untagged ones fall back to the LLM. (Manual tag modes keep their own UI; the card is aimed at the automatic flow.) Per-page P10 from the phone is preserved.
- **Trailing "Unread" tag (uncommitted, after v3.2.1):** every output PDF **and** exported original image gets `Unread` as its **last** Finder tag, but **only in real-tagging modes** (`.automatic`/`.autoDate`/`.autoDateManualSeg`/`.human`) — never for **No tagging** or **Copy source tags**. Implemented as a write-once `MacOSTagger.stampUnread` flag armed from `TaggingMode.stampsUnread` via a `didSet` on `OCRProcessor.taggingMode`; the single `MacOSTagger.applyTags` primitive dedups any incoming `Unread` and appends it last (color tag stays first). Verified on real files: color-first + Unread-last, dedup on re-apply, `["Unread"]` for empty input when stamping, and no `Unread` at all when off.
- **Tagging-phase speedup (uncommitted, after v3.2.1):** the segment-tagging LLM calls ran one-at-a-time (serial). Now they run **concurrently (bounded pool of 6)** and **skip thinking** (`thinkingLevel: nil`) since tagging is a simple text→JSON task. Applies to automatic tagging (`performTaggingPhase` + new `applyGeneratedTags` helper) and the `.autoDate` date prefetch (`prefetchManualDates`). Big speedup when a run has many segments; each call is small text (no images), so it's latency-bound and parallelizes well.
